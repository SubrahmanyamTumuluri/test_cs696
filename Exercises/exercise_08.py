...
def compute(**kwargs):
    ...
    if kwargs['action'] == 'sum':
        Sum_Calc=(sum(kwargs['input']))
        print(Sum_Calc)
        if kwargs.get('return_float', False):
            print(float(Sum_Calc))
    if kwargs['action'] == 'mean':
        mean_calc=sum(kwargs['input'])/ len(kwargs['input'])
        # print(mean_calc)
        if kwargs.get('return_float', False):
           print(float(mean_calc))
          # return
compute(input=[1, 2, 3, 4, 5], action='sum', return_float='True')
compute(input=[1, 2, 3, 4, 5], action='mean', return_float='True')
